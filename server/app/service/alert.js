...
class AlertService extends Service {
   ...
   * @param {number} userId - 用户ID
   * @param {number} alertId - 提醒ID
   * @return {Object|null} 提醒详情，如果不存在或无权限则返回null
   */
      ...
      try {
        ...
        if (triggered) {
          ...
          await ctx.model.AlertHistory.create({
            alertId: alert.id,
            triggeredValue,
            message: `${alert.stockName}(${alert.stockCode}) ${this.getConditionName(
              alert.condition
            )} ${alert.value}`,
            isRead: false,
            createdAt: new Date(),
      ...
      try {
        ...
        if (triggered) {
          ...
          await ctx.model.WatchlistAlertHistory.create({
            alertId: alert.id,
            triggeredValue,
            message: `${alert.stockName}(${alert.symbol}) ${this.getConditionName(
              alert.condition
            )} ${alert.value}`,
            isRead: false,
            createdAt: new Date(),
