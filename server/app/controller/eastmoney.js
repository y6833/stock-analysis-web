...
class EastMoneyController extends Controller {
  ...
  async test() {
    ...
    try {
      ...
      const response = await axios.get(
        'http://push2.eastmoney.com/api/qt/stock/get?secid=1.000001&fields=f43,f57,f58,f169,f170,f46,f44,f45,f168,f47,f48,f60,f49,f171,f50,f51,f52,f59',
        {
          headers: {
            'User-Agent':
  ...
  async quote() {
    ...
    try {
      ...
      // 请求东方财富API
      const response = await axios.get(
        `http://push2.eastmoney.com/api/qt/stock/get?secid=${secid}&fields=f43,f57,f58,f169,f170,f46,f44,f45,f168,f47,f48,f60,f49,f171,f50,f51,f52,f59,f85,f84,f116,f86`,
        {
          headers: {
            'User-Agent':
              'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
    ...
    try {
      ...
      try {
        ...
        const shResponse = await axios.get(
          'http://83.push2.eastmoney.com/api/qt/clist/get?pn=1&pz=5000&fs=m:1+t:2,m:1+t:23&fields=f12,f14,f100,f3',
          {
            headers: {
              'User-Agent':
                'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
        ...
        // 获取深市A股列表
        const szResponse = await axios.get(
          'http://83.push2.eastmoney.com/api/qt/clist/get?pn=1&pz=5000&fs=m:0+t:6,m:0+t:80&fields=f12,f14,f100,f3',
          {
            headers: {
              'User-Agent':
                'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
        ...
        // 获取北交所股票列表
        const bjResponse = await axios.get(
          'http://83.push2.eastmoney.com/api/qt/clist/get?pn=1&pz=500&fs=m:0+t:81+s:2048&fields=f12,f14,f100,f3',
          {
            headers: {
              'User-Agent':
                'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
        ...
        const indexResponse = await axios.get(
          'http://83.push2.eastmoney.com/api/qt/clist/get?pn=1&pz=100&fs=m:1+s:2,m:0+t:5&fields=f12,f14,f100,f3',
          {
            headers: {
              'User-Agent':
                'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
        ...
        if (indexResponse.data && indexResponse.data.data && indexResponse.data.data.diff) {
          ...
          for (const stock of Object.values(indexStocks)) {
            if (stock.f12 && stock.f14) {
              const market =
                stock.f12.startsWith('0') || stock.f12.startsWith('3') ? '深圳' : '上海'
  ...
  async search() {
    ...
    try {
      // 请求东方财富搜索API
      const response = await axios.get(
        `http://searchapi.eastmoney.com/api/suggest/get?input=${encodeURIComponent(
          keyword
        )}&type=14&token=D43BF722C8E33BDC906FB84D85E326E8`,
        {
          headers: {
            'User-Agent':
              'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
  ...
  async history() {
    ...
    try {
      ...
      // 请求东方财富K线数据API
      const response = await axios.get(
        `http://push2his.eastmoney.com/api/qt/stock/kline/get?secid=${secid}&fields1=f1,f2,f3,f4,f5,f6&fields2=f51,f52,f53,f54,f55,f56,f57,f58,f59,f60,f61&klt=${klt}&fqt=${fqt}&end=20500101&lmt=${lmt}`,
        {
          headers: {
            'User-Agent':
              'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
  ...
  async news() {
    ...
    try {
      ...
      const news = await eastmoneyService.getFinancialNews(newsCount)
      ...
      ctx.body = {
        ...
        data_source: '东方财富API',
        data_source_message: '东方财富新闻API暂时不可用',
      }
    } catch (error) {
      ctx.status = 500
      ctx.body = {
